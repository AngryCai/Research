...
'''
keys = ['Iris', 'cotton', 'vowel']  # # 'Iris', 'Wine', 'wdbc', 'cotton',
...
if __name__ == '__main__':
    for key in keys:
        ...
        acc_outer = []
        for n_rsm in range(5, 35, 5):
            acc_clf = []
            for j in np.arange(5):
                clf = HE_ELM_RMS([[BaseELM(20, dropout_prob=0.5)] * 20, [BaseELM(20, dropout_prob=0.5)] * 20],
                                 RidgeClassifier(), n_rsm=n_rsm, is_nonmlize=False)
        ...
        mean, std = np.asarray(acc_outer).mean(axis=1), np.asarray(acc_outer).std(axis=1)
        print('mean:%s\nstd:%s' % (mean, std))
