...
class KELM(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, C=1., kernel='linear', gamma=1.):
        self.C = C
        self.kernel = kernel
        self.gamma = gamma
    ...
    def fit(self, X, y):
        ...
        if y.shape.__len__() != 2:
            ...
            self.n_classes_ = self.classes_.__len__()
        # , gamma=self.gamma
        K_tr_matrix = pairwise_kernels(X, X, metric=self.kernel, gamma=self.gamma)
    ...
    def predict(self, X, prob=False):
        X = copy.deepcopy(X)
        K_ts_matrix = pairwise_kernels(X, self.X, metric=self.kernel, gamma=self.gamma)
...
parameters = {'C': [1e-3, 1e2, 1e-1, 1, 10, 100, 1000, 10000], 'gamma':[1e0, 1e-4, 1e-3, 1e2, 1e-1, 1, 10, 100]}
clf = GridSearchCV(eml, parameters, cv=3)
