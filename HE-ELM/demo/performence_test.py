    ...
    for key in keys:
        ...
        if key == 'abalone' or key == 'letter' or key == 'pen' or key == 'shuttle':
            ...
            for c in range(10):
                ...
                '''MultiELM-Feature'''
                # melm = MultiELM(BaseELM(n_hidden, dropout_prob=0.8), KELM(C=C, kernel='linear'), n_unit=n_unit)
                ...
                time_elm = round(time.clock() - start, 3)
                ...
                time_elm_ab = round(time.clock() - start, 3)
                acc_elm_ab = accuracy_score(y_test, y_pre_elm_ab)

                '''ML-ELM'''
                ml_elm = ML_ELM(2, (20, 50))
                start = time.clock()
                ml_elm.fit(X_train, y_train)
                y_pre_mlelm = ml_elm.predict(X_test)
                acc_ml_elm = accuracy_score(y_test, y_pre_mlelm)
                time_mlelm = round(time.clock() - start, 3)

                '''MLP'''
                mlp = MLP(batch_size=24, epochs=1000)
                start = time.clock()
                mlp.fit(X_train, y_train)
                y_pre_mlp = mlp.predict(X_test)
                acc_y_pre_mlp = accuracy_score(y_test, y_pre_mlp)
                time_mlp = round(time.clock() - start, 3)

                accs.append([acc_melm, acc_elmbase,  acc_elm_ab, acc_ml_elm, acc_y_pre_mlp])
                times.append([time_ours, time_elm,  time_elm_ab, time_mlelm, time_mlp])
        else:
            continue
       ...
       ############# original code position##################
        ...
        print '\tOurs\t', '\tELM\t',  '\tAdaBoost\t', '\tML_ELM\t', '\tMLP\t'
        print 'AVG:', np.round(res.mean(axis=0) * 100, 2)[0], '+-', np.round(res.std(axis=0) * 100, 2)[0], '  ', \
            ...
            # np.round(res.mean(axis=0) * 100, 2)[5], '+-', np.round(res.std(axis=0) * 100, 2)[5]
        # np.round(res.mean(axis=0) * 100, 2)[4], '+-', np.round(res.std(axis=0) * 100, 2)[4]
