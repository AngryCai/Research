...
class EMO_AE_ELM:
    ...
    def __init__(self, n_hidden, max_iter=1000, n_pop=50):
        """
    ...
    def fit(self, X, y):
        ...
        algorithm = NSGAII(Objectives(num_variables, 2, self.X, y_bin, self.n_hidden), population_size=self.n_pop)
    ...
    def get_best_index(self):
        ...
        for s in self.evo_result:  # self.evo_result
            objectives_.append(s.objectives)
        objectives_ = np.asarray(objectives_)
        index = np.asarray(objectives_)[:, 1].argmin()
...
class Objectives(Problem):
    ...
    def __init__(self, num_variables, num_objectives, X, T, n_hidden, sparse_degree=0.05, num_constraints=0):
        ...
        self.types[:] = Real(-.5, .5)  # specify the type(coding scheme) of decision variables
        self.X, self.T = X, T
        self.n_hidden = n_hidden
